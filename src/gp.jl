...
function negloglik(gp::GaussProcess, x, y)
	...
	Σ = gp.kernel(x) + Diagonal(gp.noise[1]*ones(size(x, 2)))
...
"""
# inference
function predict(gp::GaussProcess, x, x_old, y_old)
	μn = ndims(y_old)>1 ? gp.mean(x) : reshape(gp.mean(x), size(x, 2))
	μo = reshape(gp.mean(x_old), size(y_old))
	Σ_oo = gp.kernel(x_old) + Diagonal(gp.noise[1]*ones(size(x_old, 2)))
